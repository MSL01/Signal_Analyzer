...
class SignalAnalyzer(QMainWindow):
    ...
    def calculate_fft(self):
        """
        Calculate and display FFT of selected signals
        """
        if self.df is None or self.y_columns_list.count() == 0:
            return
        ...
        try:
            ...
            if len(time_data) > 1:
                ...
                actual_sampling_rate = float(self.sampling_rate_input.text())
            ...
            for i, y_column in enumerate(selected_y_columns):
                ...
                xf = fftfreq(n, 1 / actual_sampling_rate)

                # Take only positive frequencies
                positive_freq_mask = (xf > 0) & (xf <= 25)  # Limit to 0-25 Hz
                xf_positive = xf[positive_freq_mask]
                yf_positive = np.abs(yf[positive_freq_mask]) / n  # Normalize amplitude

                # Apply smoothing to reduce noise
                window_size = max(1, len(xf_positive) // 100)  # Adaptive window size
                if window_size > 1:
                ...
                # Plot FFT
                line, = self.fft_ax.plot(xf_positive, yf_smoothed,
                                         label=y_column, color=color,
                                         linewidth=float(self.line_input.text()),
                                         alpha=0.8)
                legend_handles.append(line)
            ...
            self.fft_ax.set_ylabel('Amplitude', fontsize=20)
            self.fft_ax.set_title(f'FFT Analysis {self.title_input.text()}', fontsize=16, fontweight='bold')
            self.fft_ax.grid(False)
            self.fft_ax.legend(handles=legend_handles, loc='best', framealpha=0.9, fontsize=18)

            # Set logarithmic scale for better visualization and limit x-axis to 0-25 Hz
           # self.fft_ax.set_yscale('log')
            self.fft_ax.set_xlim(0, 25)  # Fixed x-axis limit to 0-25 Hz
